/**
 * Single sitemap.xml endpoint — nahrazuje @astrojs/sitemap.
 *
 * Obsahuje všechny veřejné URL (CS + EN) s:
 * - <lastmod> pro Googlovou crawl prioritizaci
 * - <image:image> bloky pro Google Image Search
 * - <xhtml:link rel="alternate" hreflang="..."> pro mezinárodní SEO
 *
 * Priority + changefreq Google od 2017 ignoruje, ale lastmod používá.
 *
 * BUILD_DATE = "frozen" timestamp z build okamžiku. Důvod: kdyby lastmod byl `new Date()` v každém
 * requestu, Google by viděl "vše se změnilo dnes" při každém crawlu — to snižuje crawl trust
 * a Google nakonec sitemap ignoruje. Build-time konstanta odpovídá realitě statického webu.
 */
...
// Frozen v okamžiku build (Vite vyhodnotí při SSG). Aktualizuje se až dalším deployem.
...
export const GET: APIRoute = () => {
  ...
  const entries = [...staticEntries, ...csOnlyEntries, ...productEntries, ...portfolioEntries];
